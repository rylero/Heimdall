...
void DeepStreamPipeline::build() {
    ...
    // nvv4l2h264enc comes from the host JetPack installation via GST_PLUGIN_PATH mount
    ...
    if (!enc)     throw std::runtime_error("Failed to create nvv4l2h264enc — host plugin mount missing?");
